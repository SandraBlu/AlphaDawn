...
public:
	...
	TSubclassOf<class UItem> ItemClass = TSubclassOf<class UItem>();

	//Any new stacks that were created
	UPROPERTY(BlueprintReadOnly, Category = "Item Add Result")
	TArray<class UItem*> CreatedStacks = {};
	...
	static FItemAddResult AddedSome(TArray<class UItem*> InItems, const int32 InItemQuantity, const int32 ActualAmountGiven, const FText& ErrorText)
	{
	...
	static FItemAddResult AddedAll(TArray<class UItem*> InItems,const int32 InItemQuantity)
	{
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnItemRemoved, class UItem*, Item, const int32, Amount);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnItemUsed, class UItem*, Item);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnBeginLooting, class UInventoryComponent*, LootSource);
...
UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool UseItem(class UItem* Item);
	...
	bool HasItem(TSubclassOf<class UItem> ItemClass, const int32 Quantity = 1,  const bool bCheckVisibility = false) const;

	UFUNCTION(BlueprintPure, Category = "Inventory")
	UItem* FindItemByClass(TSubclassOf<class UItem> ItemClass, const bool bCheckVisibility = false) const;

	UFUNCTION(BlueprintPure, Category = "Inventory")
	TArray<UItem*> FindItemsByClass(TSubclassOf<class UItem> ItemClass, const bool bCheckVisibility = false) const;
	
	//Return the amount of a given item that we have space for based on its weight, max stack size, etc. 
	UFUNCTION(BlueprintPure, Category = "Inventory")
	virtual int32 GetSpaceForItem(TSubclassOf<class UItem> ItemClass, FText& NoSpaceReason) const;

	/**Return true if the taker can loot the item from this inventory */
	UFUNCTION(BlueprintPure, Category = "Inventory")
	virtual bool AllowLootItem(class UInventoryComponent* Taker, TSubclassOf <class UItem> ItemClass, const int32 Quantity, FText& ErrorText) const;

	/**Return true if the storer can store the given item in this inventory */
	UFUNCTION(BlueprintPure, Category = "Inventory")
	virtual bool AllowStoreItem(class UInventoryComponent* Storer, TSubclassOf <class UItem> ItemClass, const int32 Quantity, FText& ErrorText) const;

	/** Remove the item from us, and give it to the taker. Return how much of the item was able to be looted */
	virtual FItemAddResult PerformLootItem(class UInventoryComponent* Taker, TSubclassOf <class UItem> ItemClass, const int32 Quantity = 1);

	/** Remove the item from the storer, and give it to us. Return how much of the item was able to be stored*/
	virtual FItemAddResult PerformStoreItem(class UInventoryComponent* Storer, TSubclassOf <class UItem> ItemClass, const int32 Quantity = 1);
...
protected:
	...
	virtual void ServerUseItem(class UItem* Item);
	
	UFUNCTION(Server, Reliable)
	virtual void ServerConsumeItem(class UItem* Item, const int32 Quantity);

	UFUNCTION(Server, Reliable)
	virtual void ServerRemoveItem(class UItem* Item);

	//Internal, not exposed to BP;  Try Add item or Try add item from class functions will call this internal function
	FItemAddResult TryAddItem_Internal(TSubclassOf<class UItem> ItemClass, const int32 Quantity = 1);
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool RequestLootItem(class UItem* ItemToLoot, FText& ErrorText, const int32 Quantity = 1);

	UFUNCTION(Server, Reliable)
	virtual void ServerRequestLootItem(class UItem* ItemToLoot, const int32 Quantity);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Inventory")
	virtual bool RequestStoreItem(class UItem* ItemToStore, FText& ErrorText, const int32 Quantity = 1);

	UFUNCTION(Server, Reliable)
	virtual void ServerRequestStoreItem(class UItem* ItemToLoot, const int32 Quantity);

	/**Sets our loot source. Only the server can call this for obvious reasons! */
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "Inventory")
	virtual void SetLootSource(class UInventoryComponent* NewLootSource);
	
private:

	// Don't call Items.Add() directly. Use this instead, as it handles ownership
	UItem* AddItem(TSubclassOf<class UItem> ItemClass, const int32 Quantity);

	UFUNCTION()
	void OnRep_Items(const TArray<class UItem*>& OldItems);
	
	UFUNCTION()
	void OnRep_Currency(const int32 OldCurrency);

	UFUNCTION()
	void OnRep_LootSource(class UInventoryComponent* OldLootSource);
